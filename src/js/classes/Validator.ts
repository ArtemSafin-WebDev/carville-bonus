...
const defaultLocalization: Localization = {
  ru: {
    ...
    equalField: "Пароль должен совпадать",
  },
  en: {
    ...
    equalField: "Password must match",
  },
};
...
class Validator {
  ...
  constructor(form: HTMLFormElement, private scope: HTMLElement | null = null) {
    this.form = form;
    this.form.noValidate = true;
    if (!scope) {
      this.scope = this.form;
    } else {
      this.scope = scope;
    }
    ...
    this.selects = Array.from(
      this.scope.querySelectorAll("[data-required-select]")
    );

    this.checkboxes = Array.from(
      this.scope.querySelectorAll<HTMLInputElement>('input[type="checkbox"]')
    ).filter((item) => !item.closest("[data-required-select]"));
  ...
  private validateTextField(field: HTMLInputElement): ValidationError | null {
    ...
    if (value && field.matches("[data-equals-to]")) {
      const selector = field.getAttribute("data-equals-to");
      if (selector) {
        const correspondingInput =
          document.querySelector<HTMLInputElement>(selector);
        if (correspondingInput) {
          if (field.value !== correspondingInput.value) {
            this.errors.push({
              element: field,
              message: this.localization[this.locale].equalField,
            });
          }
        }
      }
  ...
  private showSelectFieldMessage(
    ...
    if (parent) {
      this.placeErrorMessage(select.parentElement, error);
    } else {
